...
kbl(Tab, align = "c", caption = "Model Features") %>%
  kable_classic("striped", full_width = F)  %>%
  row_spec(0, font_size = 10) %>%
  add_header_above(c(" "=2, "Unrestricted" = 1, "Restricted" = 1, "Unrestricted" = 1, "Restricted" = 2, " " = 11)) %>%
 add_header_above(c(" "=2, "Individual" = 2, "Joint" = 3, " "=2, "Individual" = 1, "Joint" = 1, " "=2, "P only" = 1, "P and Q" = 1, " " = 3))  %>%
  add_header_above(c( " "=2, "P-dynamics"= 5, " "=2, "Q-dynamics"= 2, " "=2, "Sigma matrix estimation" = 2, " "=2, "Dominant Country"=1), bold = T) %>%
pack_rows("Unrestricted VAR", 1, 3 , label_row_css = "background-color: #666; color: #fff;")  %>%
pack_rows("Restricted VAR (GVAR)", 4, 5, label_row_css = "background-color: #666; color: #fff;") %>%
pack_rows("Restricted VAR (JLL)", 6, 8, label_row_css = "background-color: #666; color: #fff;")
```
...
In what follows, the focus will be devoted to describe the various pieces that are necessary to implement ATSMs. Specifically, Section \@ref(S:SectionData) describes the data-set available at this package and the set of functions that are useful to retrieve data from Excel files. Section \@ref(S:SectionInputs) exposes the necessary inputs that have to be specified by the user. In Section \@ref(S:SectionEstimation) the estimation procedure is detailed. Section \@ref(S:SectionReplication) shows how to use the [MultiATSM](*LINK TO BE ASSIGNED*) package to estimate ATSMs from scratch as well as to closely reproduce some empirical features of academic papers. To ease the exposition, throughout the next sections, the database used by CM (2021) is employed to illustrate the use of the various functions available in this package.

# Data {#S:SectionData}
 
## Package data-set{#S:SectionDataPackage}
The [MultiATSM](*LINK TO BE ASSIGNED*) package contains the four data-sets used in CM (2021). The first set of data comprises several time series of zero-coupon bonds yields from four emerging markets: China, Brazil, Mexico, and Uruguay. It is worth noting that this package requires *(i)* for estimation purposes, the maturities of the bond yields to be the same for all countries (although the function *DataForEstimation* handles different maturities across countries as inputs, the outputs generated by this same function are bond yields of the same maturities for all the economies); *(ii)* bond yields to be expressed in percentage terms (and not in basis points) per annum. The [MultiATSM](*LINK TO BE ASSIGNED*) package does not support a routine to bootstrap zero-coupon yields from coupon bonds. This data manipulation procedure must be handled by the user herself, if necessary.  
```{r}
data('CM_Yields')
```
...
Once the model parameters from the ATSM have been estimated, the [MultiATSM](*LINK TO BE ASSIGNED*) package enables the numerical and graphical compilation of the following additional outputs:
...
The function *InputsForOutputs* can provide some  guidance for customizing the features of the wished outputs. Conditional on these settings, individual folders are created to store the different types of the desired graphical outputs. 
...
The type of constraint to be set for the parameters *r0*, *se*, *K0Z* and *K1Z* is typically *bounded* and are used for bounded matrices. For *K1XQ* and *SSZ*, the [MultiATSM](*LINK TO BE ASSIGNED*) package currently provides the following options:
...
This section presents some examples on how to use the [MultiATSM](*LINK TO BE ASSIGNED*) package to fully implement ATSMs. Further on, the functions of this package are used to replicate some of the results presented in the original papers of JPS (2014) and CM (2021).  
...
Supported by BR (2017)'s data-set, the code below uses the [MultiATSM](*LINK TO BE ASSIGNED*) package to estimate the key model parameters from the ATSM along the lines of JPS (2014).
...
The tables below compare the ATSM parameter estimates generated from BR (2017) and the [MultiATSM](*LINK TO BE ASSIGNED*). Overall, one can note that the differences in the estimates are economically modest.


```{r, eval=ModelPara, echo= FALSE}
...
kableExtra::kbl(TableQ, align = "c", caption = "$Q$-dynamics parameters") %>%
  ...
  footnote(general = " $\\lambda$'s are the eigenvalues from the risk-neutral feedback matrix and $r0$ is the long-run mean of the short rate under Q.")
```
...
kableExtra::kbl(TableP, align = "c", caption = "$P$-dynamics parameters") %>%
  kable_classic("striped", full_width = F)  %>%
  row_spec(0, font_size = 14) %>%
  add_header_above(c(" "= 1, "K0Z"=1, "K1Z" = 5), bold = T) %>%
  pack_rows("BR (2017)", 1, 5) %>%
  pack_rows("MultiATSM", 6, 10) %>%
  footnote(general = " $K0Z$ is the intercept and $K1Z$ is feedback matrix from the $P$-dynamics.")
```
...
kableExtra::kbl(se, align = "c", caption ="Portfolio of yields with errors") %>%
  kable_classic("striped", full_width = F)  %>%
  row_spec(0, font_size = 14) %>%
  footnote(general = " $se$ is the standard deviation of the portfolio of yields observed with errors.")
```


It is relevant to highlight that the script above makes use of the principal component weight provided by BR (2017). Such a matrix is simply a scaled-up version of the one provided by the function *pca_weights_one_country* of this package. Accordingly, despite the numerical differences on the weight matrices, both methods generate time series of spanned factors which are perfectly correlated. Another difference between the two approaches relates to the construction form of the log-likelihood function (llk): while in the BR (2017) code the llk is expressed in terms of portfolio of yields, the [MultiATSM](*LINK TO BE ASSIGNED*) package generates this same input directly as a function of observed yields (i.e. both procedures lead to equivalent llk up to the Jacobian term).
