...
test_that("Optimization + Outputs + Graphs return correct structure (GVAR model)", {
  ...
  ATSMInputs <- InputsForOpt(
    ...
    FacLab, Economies, DataFreq, GVARlist, CheckInputs = FALSE, verbose = FALSE
  )
  ...
  res_For <- ForecastYields(ModelType, res_Opt, InputsForOutputs, FacLab, Economies, JLLlist = NULL,
...
test_that("Optimization returns correct output structure (GVAR model, unconstrained)", {
  ...
  GVARlist <- list( VARXtype = "constrained: Inflation", W_type = "Sample Mean", t_First_Wgvar = "2005",
                    t_Last_Wgvar = "2019", DataConnectedness = TradeFlows)
